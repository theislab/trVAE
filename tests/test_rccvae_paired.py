...
DATASETS = {
    "Edges2Shoes": {"name": "edges2shoes", 'source_key': 'edge', 'target_key': 'shoe',
...
def evaluate_network(data_dict=None, z_dim=100, n_files=5, k=5, arch_style=1, preprocess=True, max_size=80000):
    ...
    target_images_train = train_data[train_data.obs["condition"] == source_key].X
    target_images_valid = valid_data[valid_data.obs["condition"] == source_key].X
    ...
    target_images_train = np.reshape(target_images_train, (-1, np.prod(image_shape)))
    target_images_valid = np.reshape(target_images_valid, (-1, np.prod(image_shape)))

    source_data_train = anndata.AnnData(X=source_images_train)
    source_data_valid = anndata.AnnData(X=source_images_valid)

    target_data_train = anndata.AnnData(X=target_images_train)
    target_data_valid = anndata.AnnData(X=target_images_valid)
    ...
    for j in range(n_files):
        ...
        target_sample_train = target_data_train.X[random_samples_train]
        target_sample_valid = target_data_valid.X[random_samples_valid]

        source_sample_train = np.array(source_sample_train)
        source_sample_valid = np.array(source_sample_valid)

        target_sample_train = np.array(target_sample_train)
        target_sample_valid = np.array(target_sample_valid)
...
def visualize_trained_network_results(data_dict, z_dim=100, arch_style=1, preprocess=True, max_size=80000):
    ...
    train_data.obs.loc[train_data.obs['condition'] == source_key, 'condition'] = f'Edges'
    train_data.obs.loc[train_data.obs['condition'] == target_key, 'condition'] = f'Shoe'
