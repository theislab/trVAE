...
def test_train_whole_data_one_celltype_out(data_name="pbmc",
    ...
    for cell_type in train.obs[cell_type_key].unique().tolist():
        ...
        network = rcvae.RCVAE(x_dimension=net_train_data.X.shape[1], z_dimension=z_dim, alpha=alpha, beta=beta,
                              kernel=kernel, train_with_fake_labels=False,
                              model_path=f"./")
        ...
        true_labels, _ = rcvae.label_encoder(net_train_data)
        ...
        rcvae.plotting.reg_mean_plot(all_adata, condition_key=condition_key,
                                     axis_keys={"x": ctrl_key, "y": stim_key, "y1": "pred"},
                                     gene_list=diff_genes,
                                     path_to_save=f"./figures/reg_mean_{z_dim}.pdf")
        rcvae.plotting.reg_var_plot(all_adata, condition_key=condition_key,
                                    axis_keys={"x": ctrl_key, "y": stim_key, 'y1': "pred"},
                                    gene_list=diff_genes,
...
def reconstruct_whole_data(data_name="pbmc", condition_key="condition"):
    ...
    for idx, cell_type in enumerate(train.obs[cell_type_key].unique().tolist()):
        ...
        os.chdir(f"./results/{data_name}/{cell_type}")
        net_train_data = train[~((train.obs[cell_type_key] == cell_type) & (train.obs[condition_key] == stim_key))]
        network = rcvae.RCVAE(x_dimension=net_train_data.X.shape[1], z_dimension=50, alpha=0.001, beta=100,
