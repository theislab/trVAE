...
def plot_boxplot(data_dict,
    ...
    train_data, _ = merge_data(data_dict)
    recon_data = sc.read(f"../data/reconstructed/RCVAEMulti/{data_name}.h5ad")
    ...
    path_to_save = f"../results/RCVAEMulti/Benchmark/{data_name}/"
    os.makedirs(path_to_save, exist_ok=True)

    sc.settings.figdir = path_to_save

    diff_genes = score(train_data, n_deg=10 * n_genes, n_genes=500, cell_type_key=cell_type_key,
    ...
    if not restore:
        n_cell_types = len(train_data.obs[cell_type_key].unique().tolist())
        all_scores = np.zeros(shape=(n_cell_types * 10 * n_genes, 1))
        for bin_idx in range(10):
            for cell_type_idx, cell_type in enumerate(train_data.obs[cell_type_key].unique().tolist()):
...
def stacked_violin_plot(data_dict, method, score_type="median_score"):
    ...
    diff_genes = score(train_data, n_deg=10, n_genes=500, cell_type_key=cell_type_key, conditions=conditions,
                       sortby=score_type)
    diff_genes = diff_genes["genes"].tolist()
...
def reconstruct_whole_data(data_dict={}, z_dim=100):
    ...
    for idx, cell_type in enumerate(cell_types):
        ...
        network = rcvae.RCVAEMulti(x_dimension=train_data.shape[1],
                                   z_dimension=z_dim,
                                   n_conditions=3,
                                   model_path=f"../models/RCVAEMulti/{data_name}/{cell_type}/{z_dim}/",
                                   )
        ...
        cell_type_data = train_data[train_data.obs[cell_type_key] == cell_type]
        cell_type_ctrl_data = train_data[
            ((train_data.obs[cell_type_key] == cell_type) & (train_data.obs["condition"] == ctrl_key))]
    ...
    all_data.write_h5ad(f"../data/reconstructed/RCVAEMulti/{data_name}.h5ad")
...
def visualize_trained_network_results(data_dict, z_dim=100):
    ...
    for cell_type in cell_types:
        ...
        rcvae.plotting.reg_mean_plot(cell_type_adata,
                                     ...
                                     labels={'x': 'pred hpoly from ctrl', 'y': 'real hpoly'},
                                     legend=False,
                                     fontsize=20,
                                     textsize=14,
                                     title=cell_type,
...
def visualize_trained_network_results_multimodal(data_dict, z_dim=100):
    ...
    data, _ = merge_data(data_dict)
    path_to_save = f"../results/RCVAEMulti/{data_name}/{z_dim}/{source_key} to {target_key}/Visualizations/"
    os.makedirs(path_to_save, exist_ok=True)
    sc.settings.figdir = os.path.abspath(path_to_save)
    ...
    network = rcvae.RCVAEMulti(x_dimension=data.shape[1],
                               z_dimension=z_dim,
                               n_conditions=3,
                               model_path=f"../models/RCVAEMulti/{data_name}/{z_dim}/", )
    network.restore_model()
    if sparse.issparse(data.X):
        data.X = data.X.A
